...
다섯 포지션을 한꺼번에 외우려고 하면 부담이 큽니다. 그래서 실전에서 가장 많이 쓰이는 접근법은 **두 박스만 익히는 것**입니다. F 마이너 펜타토닉의 1포지션과 3포지션은 서로 한 음씩 겹쳐 있어서, 두 박스가 맞물려 지판의 절반 이상을 덮습니다. 그 위로는 같은 모양이 옥타브 단위로 반복되기 때문에, 결국 **두 모양**만 손에 익혀두면 지판 전체가 손에 들어옵니다.
...
notes: F, Ab, Bb, C, Eb
positions: 1-5, 3-7
position-labels: A형 (1포지션), B형 (3포지션)
\`\`\`

좌우 화살표로 두 박스를 비교해보세요. 1포지션의 위쪽 음 두 개가 3포지션의 아래쪽 음 두 개와 같은 프렛 자리에 놓입니다. 이 **겹침 구간**이 두 박스를 자연스럽게 이어주는 통로 역할을 합니다.

### 어떻게 연습하는가

1. **A형부터** — 1포지션 박스를 박자 안에서 위·아래로 자유롭게 연주합니다
2. **B형 추가** — 3포지션 박스도 같은 방식으로 외웁니다
3. **연결 연습** — 한 박스에서 시작해서 겹침 음을 지나 다른 박스로 흘러가는 것을 시도합니다
4. **옥타브 위로** — 두 박스를 12프렛 위로 그대로 이동하면 지판 전체가 됩니다

\`\`\`callout
type: tip
title: 핵심 통찰

5개를 다 외우려 하기보다 두 박스만 완전히 손에 익히는 게 훨씬 빠릅니다. 두 박스가 만나는 *겹침 음*을 의식하면, 솔로 중에 자연스럽게 위치를 옮기게 됩니다.
\`\`\`
