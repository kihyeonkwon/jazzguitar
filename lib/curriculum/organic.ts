...
## 두 모양만 외우면 지판 전체가 풀립니다

5개 포지션을 통째로 외우는 대신, **딱 두 가지 작은 모양**만 익히고 그것을 지판 위에 차례로 쌓아 올리는 방법이 있습니다. 어느 줄·어느 프렛이든 이 두 모양이 번갈아가며 반복되기 때문에, 모양의 정체만 알면 위치는 자동으로 따라옵니다.

\`\`\`shape
type: A
title: Shape A — 두 줄짜리 (3프렛 간격)
\`\`\`

Shape A는 **인접한 두 줄**에 같은 간격으로 음이 놓이는 모양입니다. R(루트)이 위쪽 줄의 왼쪽에 오고, 같은 줄 오른쪽 끝과 아래쪽 줄의 양 끝에도 음이 하나씩 자리잡습니다. 박스 외곽에 표시된 ♭5는 이 모양을 다음 모양으로 *이어주는 통로*입니다.

\`\`\`shape
type: B
title: Shape B — 세 줄짜리 (안쪽 줄)
\`\`\`

Shape B는 **세 줄에 걸친 작은 사각형**입니다. R이 가운데 줄 오른쪽에 자리잡고, 세 줄에 음이 골고루 분포합니다. ♭5는 이 모양의 *안쪽*에 들어와 있어서, 솔로 중에 자연스럽게 블루 노트를 끼워 넣을 수 있습니다.

## 어떻게 쌓이는가

지판을 6번줄 낮은 음부터 1번줄 높은 음까지 거슬러 올라가면, 이 두 모양이 다음처럼 차례로 등장합니다:

1. **6번줄 → 5번줄 영역**: Shape A 한 번
2. **5번줄 → 4번줄 → 3번줄 영역**: Shape B 한 번
3. **3번줄 → 2번줄 영역**: Shape A 한 번
4. **2번줄 → 1번줄 영역**: Shape B 한 번
5. 1번줄까지 도달하면, 다시 6번줄 같은 모양으로 시작해서 한 옥타브 위에서 반복

각 모양이 끝나는 자리에서 다음 모양이 *자연스럽게* 이어집니다. 그래서 위로 갈수록 손은 대각선으로 올라가게 됩니다.

\`\`\`callout
type: tip
title: 핵심 통찰

다섯 박스를 따로따로 외우려 들면 머릿속이 복잡해집니다. 그런데 *두 모양만 있다*고 생각하면, 지판 위의 어떤 자리든 "지금은 A인가 B인가"만 판단하면 됩니다. 위치 외우기에서 *모양 인식*으로 사고가 바뀌는 것이 핵심입니다.
\`\`\`
